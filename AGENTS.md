...
- **Admin panel backend**: Routes via `/api/config`, `/api/admin-login`, `/api/admin-data`, `/api/admin-mutate`, `/api/admin-orders-sync`. `src/lib/supabase.ts` proxies writes through `adminMutate`; auth stored in `localStorage` (`bb_admin_auth` etc.).
