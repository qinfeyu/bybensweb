...
export const OrdersPage: React.FC<OrdersPageProps> = ({
  ...
  return (
    <div className="space-y-6">
                ...
                <div key={o.id} className={`p-4 space-y-3 transition-colors ${isSelected ? 'bg-red-50/40' : 'hover:bg-slate-50/60'}`}>
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      ...
                      />
                      <span className="font-black text-slate-900 text-sm truncate max-w-[45%]">{o.id}</span>
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold inline-flex items-center gap-1 border shrink-0 ${
                        sLower === 'pre-order'
      ...
      {selectedOrderIds.length > 0 && (
        <div className="fixed bottom-24 sm:bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900 text-white px-4 py-3 rounded-2xl shadow-2xl border border-slate-700 flex items-center justify-between gap-3 max-w-lg w-[92vw] animate-in slide-in-from-bottom-4 flex-wrap">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <div className="p-4 sm:p-6 space-y-5 text-xs overflow-y-auto flex-1 thin-scrollbar">
              {/* Customer Info */}
                          ...
                          return (
                            <div key={idx} className="flex items-center gap-2 p-2.5 border-b border-emerald-100 last:border-0 bg-white/70">
                              ...
                              <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden bg-white shrink-0">
                                <button onClick={() => editItemQty(idx, -1)} className="px-3 py-2 hover:bg-slate-100 font-bold text-slate-600">−</button>
                              ...
                              <button onClick={() => removeEditItem(idx)} className="text-rose-600 hover:text-rose-800 p-2 shrink-0" title="Remove item">
                                <Trash2 className="w-4 h-4" />
                              </button>
                            </div>
                          );
