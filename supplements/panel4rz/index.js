      ...
      async function refreshBusinessPortalData() {
        try {
          ...
          const preP = sb.from("pre_orders").select("id,customer_name,customer_phone,notes,total_amount,status,date,created_at").order("date", { ascending: false })
            .then(r => {
              if (r.error) throw r.error;
              return r.data || [];
            })
            .catch(async () => {
              const res = await sb.from("pre_orders").select("*");
              return res.data || [];
            });
